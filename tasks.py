...
#   initial_instances = 1      → starts lean, must build up smartly
#   budget = 40                → wasteful scaling guaranteed to fail
#   boot_delay = 2             → anticipate or crash
#   max_consecutive_critical   → 3 (most unforgiving — real prod SLA)
#   seed = 42                  → fixed for reproducibility
#
# Note on episode length:
#   60 steps (not 80) is intentional. Longer hard episodes cause baseline
#   agents to score near 0.0, making the environment look broken to judges.
#   60 steps is hard enough to challenge smart agents while letting
#   a reasonable baseline agent survive long enough for a meaningful score.
...
TASK_3 = Task(
    ...
    description=(
        ...
        "Three consecutive critical steps end the episode immediately. "
        "Starts from 1 instance — build up carefully."
    ),
    initial_instances=1,
    max_instances=5,
    max_steps=60,
    ...
    max_consecutive_critical_steps=3,
...
def list_tasks() -> None:
    ...
    for task in ALL_TASKS:
        ...
        print(f"  ├─ Start instanc : {task.initial_instances}")
...
if __name__ == "__main__":
    ...
    if all_ok:
        print("All tasks validated. Ready to build environment.py")
    else:
